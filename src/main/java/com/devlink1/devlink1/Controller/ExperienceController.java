...
//        save the experience
    ...
    @GetMapping("/addNew")
    public String addNewExperienceForm(Experience exp, Model model  ){

        model.addAttribute("exp", exp);

        return "experience-addNew";
    }

    @PostMapping("/add")
    public String addNewExperience(@AuthenticationPrincipal UserDetails userDetails ,
            @ModelAttribute("exp") Experience exp){

        // Get the current user
        User user = userService.findByUsername(userDetails.getUsername());
        System.out.println("The username "+ user.getUsername() + " is about to add a new experience");

        // link the experience object to the current user
        exp.setUser(user);
        System.out.println("New experience is linked with the username "+ user.getUsername() + " with id " + user.getId());

        experienceService.addExperience(exp);
        System.out.println("New experience with id "+exp.getId()+" is added");
